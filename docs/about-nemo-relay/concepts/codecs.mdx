...
Normalized request messages preserve `system` and `developer` as distinct
instruction roles. Structured output uses an optional typed
`response_format` with `json_object` or `json_schema` semantics. Provider
codecs retain unmodeled containing-object fields in `native_wrapper` and
unmodeled format-descriptor fields in `native_format`. Unknown native format
kinds remain in generic request data instead of being guessed into a typed
contract.
