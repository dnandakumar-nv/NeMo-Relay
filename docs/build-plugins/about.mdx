...
If an embedded Rust component also owns background tasks, queues, replay
transports, or durable work, define its stop, drain, abort, and reconciliation
behavior before registration. [Register Plugin Behavior](/build-plugins/language-binding/register-behavior)
documents the component shutdown hooks and the difference between immediate
and graceful clear.
