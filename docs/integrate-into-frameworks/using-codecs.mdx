...
Provider annotations include a distinct Developer message role and an optional
typed structured-response format. These are provider semantics, not typed value
codec features; do not reproduce their native mapping in an application value
codec.
